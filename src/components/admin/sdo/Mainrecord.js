...
//let  viewurl = "../updatetreefallingrecord/id=";
...
componentDidMount() {
    ...
    axios.get('fellingTree').then(res =>  {
    ...
    return (
        <>
          <div className="row">
                ...
                <Link to="/ApprovedRejectedApplications"  className='me-xxl-5 ms-auto w-auto'>
                    <button className='btn' style={{backgroundColor: "#e91e63", color: "#fff"}}>Approved/Rejected Application</button>
                </Link>
            </div>
                            ...
                            this.state.data.map((result, element) => {
                                return (
                                    <tr key={result.tree_feeling_id}>
                                        ...
                                        <td>{result.SRN}</td>
                                        <td className='pending-application'>{result.status}</td>
